import { BUILDINGS, BUILDING_BY_ID, RESOURCE_META, applyProduction, getActiveUpgrade, getBuildingPowerAtLevel, getCapacity, getCharacterLevel, getClaimableForBuilding, getLevel, getPower, getUpgradeReadiness, } from "/shared/game-config.js";
const TICK_MS = 1000;
...
function getBuildingSprite(buildingId, level) {
    ...
    if (!variants || variants.length === 0 || level <= 0)
        return null;
...
function getSpriteHitCanvas(spriteEl) {
    ...
    if (!src || !spriteEl.naturalWidth || !spriteEl.naturalHeight)
        return null;
    const cacheKey = `${src}::${spriteEl.naturalWidth}x${spriteEl.naturalHeight}`;
    if (spriteHitCache.has(cacheKey))
        return spriteHitCache.get(cacheKey);
...
function isSpritePixelHit(event, spriteEl) {
    if (!spriteEl || spriteEl.classList.contains("hidden"))
        return false;
    if (event.detail === 0)
        return true;
...
function buildMapNodes() {
    ...
    for (const building of BUILDINGS) {
        ...
        buildingButton.addEventListener("click", async (event) => {
            if (!state)
                return;
            if (!isSpritePixelHit(event, spriteEl))
                return;
